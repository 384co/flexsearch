...
;function ka(a, b, c, d) {
  ...
  for (let m = e - 1; 0 <= m; m--) {
    ...
    let n = !g;
    for (let l = 0; l < u; l++) {
      const t = p[l], z = t.length;
      if (z) {
        for (let C = 0, B, A; C < z; C++) {
          if (A = t[C], g) {
            if (g[A]) {
              if (!m) {
                if (c) {
              ...
              if (this.boost && (f = Math.min(f / this.boost(b, l, n) | 0, q - 1)), this.push_index(p, l, f, a, c), u && 1 < d && n < d - 1) {
                for (e = w(), g = this.resolution_ctx, f = l, h = Math.min(u + 1, d - n), e[f] = 1, k = 1; k < h; k++) {
                  if ((l = b[this.rtl ? d - 1 - n - k : n + k]) && l.length >= this.minlength && !e[l]) {
                    e[l] = 1;
                    const t = M(g + (d / 2 > g ? 0 : 1), d, n, h - 1, k - 1), z = this.bidirectional && l > f;
                    this.push_index(m, z ? f : l, t, a, c, z ? l : f);
                  }
                }
              }
          }
  ...
  if (b) {
    switch(x(b) && (b = JSON.parse(b)), console.log(a), a) {
      ...
      case "reg":
        ...
        break;
      case "map":
        0 === Object.keys(this.map[0]).length ? this.map = b : (this.map.forEach((c, d) => {
          for (let e in this.map[d]) {
            b[d].hasOwnProperty(e) && (this.map[d][e] = this.map[d][e].concat(b[d][e]));
          }
        }), b.forEach((c, d) => {
          for (let e in b[d]) {
...
R.prototype.search = function(a, b, c, d) {
  ...
  for (let l = 0, t, z, C; l < k.length; l++) {
    let B;
    z = k[l];
    x(z) || (B = z, z = B.field, a = B.query || a, b = B.limit || b);
    if (n) {
      n[l] = this.index[z].searchAsync(a, b, B || c);
    } else {
      C = (t = d ? d[l] : this.index[z].search(a, b, B || c)) && t.length;
      if (m && C) {
        const A = [];
        let H = 0;
        p && (A[0] = [t]);
        for (let W = 0, oa, Q; W < m.length; W++) {
          if (oa = m[W], C = (Q = this.tagindex[oa]) && Q.length) {
            H++, A[A.length] = p ? [Q] : Q;
          }
        }
